...
export const addToWishlist = async (req, res, next) => {
  try {
    ...
    if (!productId) {
      return next(
        errorHandler(STATUS_CODES.BAD_REQUEST, "productId is required")
      );
    }
...
export const removeFromWishlist = async (req, res, next) => {
  try {
    ...
    if (!wishListDB)
      return next(errorHandler(STATUS_CODES.NOT_FOUND, "wishlist not found"));
...
export const getWishlist = async (req, res, next) => {
  try {
    const userId = refreshTokenDecoder(req);
    if (!userId)
      return next(errorHandler(STATUS_CODES.NOT_FOUND, "user not found"));

    let wishlist = await wishListDB
      .findOne({ user: userId })
      .sort({ createdAt: -1 })
      .populate("products.productId");

    if (!wishlist) {
      wishlist = await wishListDB.create({ user: userId, products: [] });
    }
