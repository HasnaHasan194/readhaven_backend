...
export const addProduct = async (req, res) => {
    try {
        ...
        if (typeof regularPrice !== 'number' || regularPrice < 0) {
            return res.status(400).json({ message: "Regular price must be a non-negative number" });
        }
        ...
        if (typeof availableQuantity !== 'number' || availableQuantity < 0 || !Number.isInteger(availableQuantity)) {
            return res.status(400).json({ message: "Available quantity must be a non-negative integer" });
        }

        if (productOffer !== undefined && (typeof productOffer !== 'number' || productOffer < 0)) {
            return res.status(400).json({ message: "Product offer must be a non-negative number" });
        }
        ...
        if (!productImages || !Array.isArray(productImages)) {
            return res.status(400).json({ message: "Product images must be provided as an array" });
        }
...
export const editProduct = async (req, res) => {
    try {
        ...
        if (name) {
            ...
            if (duplicateProduct) {
                return res.status(400).json({ message: "Another product with this name already exists in the same category" });
            }
        }
        ...
        if (productImages && !Array.isArray(productImages)) {
            return res.status(400).json({ message: "Product images must be an array" });
        }
