...
export const addProduct = async (req, res) => {
    try {
        ...
        if (typeof regularPrice !== 'number' || regularPrice < 0) {
            return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Regular price must be a non-negative number" });
        }
        ...
        if (!isValidCategory) {
            return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Invalid category" });
        }
        ...
        if (existingProduct) {
            return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Product already exists" });
        }

        if (!productImages || !Array.isArray(productImages)) {
            return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Product images must be provided as an array" });
        }
...
export const getProductEdit=async(req,res)=>{
    try{
        ...
        return res.status(STATUS_CODES.SUCCESS).json({message :"product fetch successfully",product});
    }
    catch(error){
...
export const editProduct = async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Invalid product ID" });
        }
        ...
        if (name) {
            ...
            if (duplicateProduct) {
                return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Another product with this name already exists in the same category" });
            }
        }
        ...
        if (regularPrice !== undefined && (typeof regularPrice !== 'number' || regularPrice < 0)) {
            return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Regular price must be a non-negative number" });
        }

        if (availableQuantity !== undefined && (typeof availableQuantity !== 'number' || availableQuantity < 0 || !Number.isInteger(availableQuantity))) {
            return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Available quantity must be a non-negative integer" });
        }
        ...
        if (productImages && !Array.isArray(productImages)) {
            return res.status(STATUS_CODES.BAD_REQUEST).json({ message: "Product images must be an array" });
        }
